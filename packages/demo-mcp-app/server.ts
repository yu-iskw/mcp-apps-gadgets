...
export function createServer() {
  const server = new McpServer({ name: "mcp-app-gadgets-demo", version: "0.1.0" });
  registerAppTool(server, "render-metric", {
    title: "Metric card",
  ...
  }, async ({ title, value, unit }) => ({
    content: [{ type: "text", text: `${title}: ${value}${unit ? ` ${unit}` : ""}` }],
